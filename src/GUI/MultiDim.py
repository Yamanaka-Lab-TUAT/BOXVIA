...
if getattr(sys, 'frozen', False):
    ...
    if os.getcwd() == '/':
        program_directory = os.path.dirname(
            os.path.dirname(os.path.dirname(program_directory)))
else:
...
slider = [html.Div(id="dimdim_"+str(i+4), children=sliders[i],
                   style={'display': 'none'}) for i in range(n_max-3)]
...
layout = dbc.Container([
    dbc.Row([dbc.Col([html.H6('Batch size',
                      ...
                      html.Img(src='assets/help.png',
                               id="batch_text_info",
                               height='25px',
                               style={'margin-top': '2px'},
                               ),
                      dbc.Tooltip('Number of input parameter candidates suggested by BO.',
    ...
    dbc.Row([dbc.Col([html.H6('Jitter or Weight',
                      ...
                      dbc.Tooltip(['A hyperparameter of the acquisition function that determines the trade-off between local optimization and global search.',
                                   ' (A larger value encourages a global search, whereas a smaller value encourages local optimization.)'],
                                  target="jwname_info",
                                  placement='top',
                                  ),
                      ],
    ...
    dbc.Row([dbc.Col([html.H6('Kernel function',
                      ...
                      dbc.Tooltip('Type of kernel function used in Gaussian process regression. Matern 5/2, Materb 3/2, or RBF are commonly used.',
                                  target="kernel_info",
                                  placement='top',
                                  ),
                      ],
    ...
    dbc.Row([dbc.Col([dbc.Checklist(id="maximize",
                      ...
                      dbc.Tooltip('Check this option if maximization is performed.',
             ...
             dbc.Col([dbc.Checklist(id="exact_fval",
                                    options=[
                                        {'label': 'Noiseless', 'value': 'val_efval'}],
                                    value=[],
                                    style={'display': 'inline-block'},
                                    ),
                      html.Img(src='assets/help.png',
                      ...
                      dbc.Tooltip('Check this option if noiseless evaluation is available.',
             ...
             dbc.Col([dbc.Checklist(id="de_duplication",
                                    options=[
                                         {'label': 'Avoid re-evaluation', 'value': 'dup'}],
                                    value=[],
                                    style={'display': 'inline-block'},
                                    ),
                      html.Img(src='assets/help.png',
                      ...
                      dbc.Tooltip('Check this option if re-evaluation for the same data is avoided.',
    ...
    dbc.Row([dbc.Col(html.H6('Axis name',
             ...
             dbc.Col([html.H6('Range',
                      ...
                      dbc.Tooltip('Set the range of possible values for the input parameters. The range can be saved & loaded.',
                                  target="range_info",
                                  placement='top',
                                  ),
                      ],
             ...
             dbc.Col([html.H6('Interval',
                      ...
                      dbc.Tooltip('Set the discretizing interval for input parameters to be suggested.',
    ...
    dbc.Row([dbc.Col([html.H5('Suggested data table',
                      ...
                      dbc.Tooltip('This table is editable.',
             ...
             dbc.Col([dbc.Button('Export',
                      ...
                      dbc.Tooltip('Download this table as "Suggest.csv."',
    ...
    dbc.Row(dbc.Col([dbc.Button(html.Div('Add to the table of imported data',
                     ...
                     dbc.Tooltip('Add the data in this table to the bottom of the imported data table.',
                                 target="add_info",
                                 placement='bottom',
                                 ),
                     ],
    ...
    html.Div([html.H6('Display axis',
              ...
              dbc.Tooltip('Select three parameters as axes of 3D graph.',
    ...
    dbc.Row(dbc.Col(dbc.Checklist(id="axis_set",
                                  ...
                                  inline=True,
                                  style={'width': '1000px'},
                                  labelStyle={'color': 'Red', 'width': '150px'},
                                  ),
                    width=10,
                    ),
            className='mt-2',
...
def read_initdata(columns):
    ...
    options = [
        [{'label': axis_name[i], 'value': 'set_'+str(i)} for i in range(n_axis)]]
...
def load_range(n_click, filename):
    ...
    try:
        ...
        if is_file:
            ...
            maxmin = np.loadtxt(program_directory+'/'+filename
                                + '_range.csv', delimiter=',', skiprows=1, usecols=cols)
...
def runBO(n_click, data, columns, batch_size, actype, jwparam, kernel, exact_fval, de_duplication, maximize, constraint, *args):
    ...
    try:
        df = pd.DataFrame(data=data, columns=[
                          columns[i]['name'] for i in range(len(columns))])
        vmaxmin = []
        cont_or_disc = []
        interval = []
        for i in range(n_axis):
        ...
        vis.runBO_visualize(vmaxmin, actype, exact_fval, de_duplication, batch_size,
                            jwparam, constraint.splitlines(), kernel, maximize, cont_or_disc, interval)
        ...
        data = pd.DataFrame(data=suggest_points,
                            index=index, columns=df.columns)
...
@app.callback([Output("graphspace_3D", 'children'),
              ...
              [Output("value_dim_"+str(i+4), 'children')
...
def make3D(n_click, data, columns, resolution, display_type, display_plane, axis_set, marker_size, ts, datas, ts2ds, data2ds):
    ...
    if len(axis_set) == 3:
        ...
        for i in range(len(unselect_axis)):
            set_slider.append({})
            set_slider.append(
                float('{:.3g}'.format(vmaxmin[unselect_axis[i]][0])))
            set_slider.append(
                float('{:.3g}'.format(vmaxmin[unselect_axis[i]][1])))
            set_slider.append(
                (vmaxmin[unselect_axis[i]][0]-vmaxmin[unselect_axis[i]][1])/100.)
        ...
        for i in range(len(unselect_axis)):
            if ts2ds == None:
                values_s[i] = float('{:3g}'.format(
                    vmaxmin[unselect_axis[i]][1]))
            else:
        ...
        for i in range(len(unselect_axis)):
            slider_text.append([html.Div(axis_name[unselect_axis[i]]+' = ',
                                ...
                                dcc.Input(id="dim_value_"+str(i+4),
                                          type='number',
                                          max=float('{:3g}'.format(
                                              vmaxmin[unselect_axis[i]][0])),
                                          min=float('{:3g}'.format(
                                              vmaxmin[unselect_axis[i]][1])),
                                          value=values_s[i],
                                          style={
                                              'width': '10%', 'display': 'inline-block', 'margin-left': '10px'},
                                          debounce=True,
                                          ),
                                ],
                               )
        ...
        for i in range(unselect_max-len(unselect_axis)):
            slider_text.append(
                html.Div(id="dim_value_"+str(i+len(unselect_axis)+4)))
...
@app.callback([Output("dim_"+str(i+4), 'value') for i in range(unselect_max)],
              [Input("dim_value_"+str(i+4), 'value')
               for i in range(unselect_max)],
              prevent_initial_call=True)
